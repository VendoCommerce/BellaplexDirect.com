...
namespace CSWeb.AU.UserControls
{
    public partial class BillingCreditForm : System.Web.UI.UserControl
    {
        ...
        protected void rblBillingDifferent_CheckedChanged(object sender, EventArgs e)
        {
            bool billingVal = Convert.ToBoolean(rblBillingDifferent.SelectedItem.Value);
            if (billingVal)
        ...
        {
            if (Convert.ToBoolean(rblBillingDifferent.SelectedItem.Value))
            {
        ...
        public void SaveData()
        {
            ...
            if (rId == 0)
            {
                if (Convert.ToBoolean(rblBillingDifferent.SelectedItem.Value)) //override if the user select yes button
                {
                    Address billingAddress = new Address();
                    billingAddress.FirstName = CommonHelper.fixquotesAccents(txtFirstName.Text);
                    billingAddress.LastName = CommonHelper.fixquotesAccents(txtLastName.Text);
